...
namespace litsdk
{
    ...
    public sealed class ActivityContext
    {
        ...
        public System.Data.DataTable GetTable(string varName)
        {
            ...
            {
                if (varinfo.TableValue == null) varinfo.TableValue = new DataTable();
        ...
        /// 执行生成的组件
        /// </summary>
        public Func<string,List<Variable>, List<Variable>> RunProtectProject { get; set; }
            ...
            if (variable != null)
            {
                switch (variable.VariableType)
                {
                    ...
                    case VariableType.Table:
                        if (variable.TableValue != null) variable.TableValue.Rows.Clear();
                        break;
                }
            }
        }
                ...
                switch (v.VariableType)
                {
                    ...
                    case VariableType.Table:
                        ...
                        else
                        {
                            ...
                            if (!string.IsNullOrEmpty(v.InitStrValue))
                            {
                                DataTable dt = null;
                                try
                                {
                                    dt = Newtonsoft.Json.JsonConvert.DeserializeObject<DataTable>(v.InitStrValue);
                                }
                                catch { }
                                if (dt != null) v.TableValue = dt;
                            }
                        }
                        break;
                }
        ...
        /// 从当前流程获取指定名称的配置
        /// </summary>
        public Func<string, List<Activity>> GetActivities;

        /// <summary>
        /// 从当前流程执行一个完整的project,是否只显示自定义日志
        /// </summary>
        public Action<Project, bool> RunProject;
